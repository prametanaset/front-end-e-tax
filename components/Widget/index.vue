<template>
  <div class="grid grid-cols-2 gap-4">
    <div class="min-w-[30%] flex flex-col justify-center md:col-span-2 lg:col-span-1">
      <BaseHeading as="h5" size="sm" weight="medium" lead="tight" class="text-muted-500 dark:text-muted-400">
        <p class="dark:text-[#B4B4B4]">{{ props.title }}</p>
      </BaseHeading>
      ...
      <div>
        <p class="text-muted-500 text-sm dark:text-[#B4B4B4]"><span class="text-muted-700 font-semibold dark:text-white">{{ percenLastMonth > 0 ? percenLastMonth : percenLastMonth * -1 }}</span>
          ใบกำกับภาษี</p>
      </div>
    </div>
