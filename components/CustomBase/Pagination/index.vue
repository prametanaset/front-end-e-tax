...
const props = withDefaults(
  defineProps<{
    ...
    /**
     * The icon to show for the first button.
     */
    firstIcon?: string

    /**
    * The icon to show for the last button.
    */
    lastIcon?: string
  ...
  }>(),
  {
    ...
    firstIcon: 'lucide:chevron-first',
    lastIcon: 'lucide:chevron-last',
    ellipsis: '…',
    classes: () => ({}),
  },
)
...
function getPaginationOutput(totalItems, itemsPerPage, currentPage) {
...
<template>
  <div class="flex items-center justify-end gap-7" :class="[
    rounded && radiuses[rounded],
    color && colors[color],
    props.classes?.wrapper,
  ]">
    <BaseFocusLoop as="ul" class="nui-pagination-list" :class="[rounded && radiuses[rounded], props.classes?.list]">
      <slot name="before-pagination"></slot>
      <div v-if="false">
        <!-- Link -->
        <li>
          <NuxtLink :to="paginatedLink(1)" tabindex="0" class="nui-pagination-link" :class="[
        ...
        <li v-if="showLastLink && pages.length > 0 && pages[0] > 2">
          <span class="nui-pagination-ellipsis" :class="[rounded && radiuses[rounded], props.classes?.ellipsis]">
            {{ props.ellipsis }}
          </span>
        </li>
        ...
        <li v-for="page in pages" :key="page">
          <NuxtLink :to="paginatedLink(page)" tabindex="0" :aria-current="currentPage === page ? 'page' : undefined"
            class="nui-pagination-link" :class="[
        ...
        <li v-if="showLastLink">
          <NuxtLink :to="paginatedLink(lastPage)" tabindex="0" class="nui-pagination-link" :class="[
      ...
      currentPage: {{ props.currentPage }} -->
        <p class="text-muted-600 text-sm font-medium pb-[2px]">
          {{ getPaginationOutput(props.totalItems, props.itemPerPage, props.currentPage) }}
        </p>
      </div>
    ...
    <BaseFocusLoop class="pt-1 flex gap-8"
      :class="[rounded && radiuses[rounded], props.classes?.buttons]">
      <slot name="before-navigation"></slot>
      
      <!-- First Page -->
      <NuxtLink :to="paginatedLink(1)" tabindex="0" :class="['link-button', { 'disabled': currentPage <= 1 }]"
        @keydown.space="(e: any) => (e.target as HTMLAnchorElement).click()"
        @click="(e: any) => handleLinkClick(e, Math.max(currentPage - 1, 1))">
      ...
      <!-- Previous -->
      <NuxtLink :to="paginatedLink(currentPage - 1)" tabindex="0" :class="['link-button', { 'disabled': currentPage <= 1 }]"
        @keydown.space="(e: any) => (e.target as HTMLAnchorElement).click()"
      ...
      <!-- Next -->
      <NuxtLink :to="paginatedLink(currentPage + 1)" tabindex="0" :class="['link-button', { 'disabled': currentPage == lastPage }]"
        @keydown.space="(e: any) => (e.target as HTMLAnchorElement).click()"
        @click="(e: any) => handleLinkClick(e, currentPage + 1)">
        <slot name="next-icon">
          <Icon :name="nextIcon" />
      ...
      <NuxtLink :to="paginatedLink(lastPage)" tabindex="0" :class="['link-button', { 'disabled': currentPage == lastPage }]"
        @keydown.space="(e: any) => (e.target as HTMLAnchorElement).click()"
        @click="(e: any) => handleLinkClick(e, currentPage + 1)">
        <slot name="last-icon">
          <Icon :name="lastIcon" />
